...
// 		setup_postdata($partners[$i]); 
		...
		$subtitle = get_field('sub_title', $partners[$i]->ID);
...
// 			$img = 'http://localhost:8888/wp-content/uploads/2017/01/20160919_Terrell_Place_0005_2_ScreenRes-170x170.jpg';
			...
			$img2 = $partner_image2['sizes']['large'];
			$class = 'grid-item--partner';
			echo '<div class="grid-item grid-item--partner effect-caption" data-slide-id="'.$partner_count.'" data-id="'.$partners[$i]->ID.'">';
			echo '<a href="#" data-name="'.get_the_title($partners[$i]->ID).'" data-title="'.$title.'" data-subtitle="'.$subtitle.'" data-bio="'.$bio.'" data-largesrc="'.$img2.'" data-resume="'.$resume['url'].'"><img src="'.$img.'" />';
